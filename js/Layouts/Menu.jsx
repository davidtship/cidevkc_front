...
const MenuItemWithChildren = ({ item, linkClassName, subMenuClassNames, activeMenuItems, toggleMenu, }) => {
    ...
    return (<li className={`nav-item ${open ? 'active' : ''}`}>
      <Link to="#" className={`nav-link ${linkClassName} ${activeMenuItems.includes(item.key) ? 'open' : ''}`} aria-expanded={open} data-menu-key={item.key} onClick={toggleMenuItem}>
...
const MenuItemLink = ({ item, className }) => {
    return (<Link to={item.url} target={item.target} className={`nav-link-ref ${className}`} data-menu-key={item.key}>
      {item.icon && <i className={item.icon}/>}
      <span> {item.label}</span>
    </Link>);
};
    ...
    return (<>
      <ul className="side-nav" ref={menuRef} id="main-menu">
        {(menuItems || []).map((item, idx) => {
            return (<React.Fragment key={idx}>
              {item.isTitle ? (<li className="nav-title">{item.label}</li>) : (<>
                  {item.children ? (<MenuItemWithChildren item={item} toggleMenu={toggleMenu} subMenuClassNames="" activeMenuItems={activeMenuItems} linkClassName="nav-link"/>) : (<MenuItem item={item} linkClassName="nav-link" className={activeMenuItems.includes(item.key) ? 'active' : ''}/>)}
                </>)}
            </React.Fragment>);
        })}
      </ul>
