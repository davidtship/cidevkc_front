    ...
    return (<Suspense fallback={<div />}>
      <div className="wrapper">
        ...
        <main className="main-content">
          <div className={`inner-content ${location.pathname.startsWith('/apps/') ? 'apps-content' : ''}`}>
            <Suspense fallback={<Preloader />}> {children}</Suspense>
          </div>
        </main>

        {!['/apps/'].some((path) => location.pathname.startsWith(path)) && (<Suspense fallback={<div />}>
            <Footer />
          </Suspense>)}
      </div>
      ...
      <Stack className="position-fixed z-1" style={{
            right: '0',
            bottom: '50%',
        }}>
        <Button onClick={handleCustomizer} variant="primary" className="btn-lg btn-icon rounded-0 rounded-start-3">
          <i className="fi fi-rr-settings fs-18"/>
        </Button>
      </Stack>

      <Stack className="support-livechat-btn position-fixed z-1" style={{ bottom: '2rem', right: '2rem' }}>
