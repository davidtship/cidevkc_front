...
const SupportLiveChat = ({ show }) => {
    ...
    return (<>
      <Card className={`support-live-chat ${show ? 'chat-show' : ''} ${isExpanded ? 'chat-expanded' : ''}`}>
        <Card.Header className="py-3 pe-2 d-flex align-items-center">
          ...
          <div className="ms-auto d-flex align-items-center">
            <Button variant="" className="btn-icon" onClick={toggleChatExpanded}>
              {isExpanded ? (<i className="fi fi-ss-compress fs-14"></i>) : (<i className="fi fi-ss-expand fs-14"></i>)}
            </Button>
            <Dropdown>
          ...
          <Card.Body>
            <div className="mt-8 mb-24 text-center mx-auto" style={{ maxWidth: '280px' }}>
              <div className="avatar-group avatar-group-lg mb-4 justify-content-center">
                {avatarImages.map((avatarSrc, index) => (<Avatar key={index} type="image" src={avatarSrc} alt={`User Avatar ${index + 1}`} className="avatar avatar-lg"/>))}
              </div>
            ...
            <div className="text-end">
              {supportLinks.map(({ url, text }, idx) => (<Link key={idx} to={url} className="d-block mb-3">
        ...
        <Card.Footer className="w-100 ps-2 pe-3 py-3 d-flex align-items-center justify-content-between">
          <Form className="w-100 d-flex align-items-center">
            <div className="w-100 mx-3 position-relative">
              <Form.Control type="text" className="rounded-pill" placeholder="Your message here...."/>
              <Stack direction="horizontal" className="align-items-center position-absolute top-50 end-0 translate-middle-y">
