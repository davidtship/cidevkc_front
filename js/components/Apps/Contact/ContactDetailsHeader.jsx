...
const ContactDetailsHeader = ({ onDelete, handlePrev, handleNext, selectedContactIndex, filteredContacts, selectedContact, onStarChange, setFilteredContacts, setSelectedContact, setContainerToggle, sidebarLeftToggle, setSidebarLeftToggle, }) => {
    ...
    return (<>
      <Stack direction="horizontal" className="px-3 border-bottom justify-content-between" style={{ minHeight: '4.5rem' }}>
        ...
        <Stack direction="horizontal" gap={1}>
          ...
          <OverlayTrigger overlay={<Tooltip>Prev</Tooltip>}>
            <Button variant="light" className="btn-icon btn-md" onClick={handlePrev} disabled={selectedContactIndex <= 0}>
              <i className="fi fi-rr-angle-left"></i>
            </Button>
          </OverlayTrigger>
          <OverlayTrigger overlay={<Tooltip>Next</Tooltip>}>
            <Button variant="light" className="btn-icon btn-md" onClick={handleNext} disabled={selectedContactIndex >= filteredContacts.length - 1}>
              <i className="fi fi-rr-angle-right"></i>
            </Button>
          </OverlayTrigger>
          <Dropdown id="dropdown-contact-actions">
            <Dropdown.Toggle variant="light" id="dropdown-contact-options" className="btn-icon btn-md arrow-none">
