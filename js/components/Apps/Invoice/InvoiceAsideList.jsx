...
const InvoiceAsideList = ({ invoiceItems, selectedInvoice, setSelectedInvoice, setShowInvoiceCreate, setLoading, searchQuery, setContainerToggle, }) => {
    ...
    return (<PerfectScrollbar className="h-100 d-flex flex-column">
      <ListGroup variant="flush" className="flex-grow-1">
        {filteredInvoiceItems.length === 0 ? (<ListGroup.Item className="py-4 text-center flex-grow-1">No items found</ListGroup.Item>) : (filteredInvoiceItems.map(({ id, invnumber, date, amount, status }, idx) => (<ListGroup.Item className={`py-4 ${selectedInvoice === invoiceItems[idx] ? 'bg-secondary-subtle' : ''}`} key={id} onClick={() => handleInvoiceItemClick(idx)} style={{ cursor: 'pointer' }}>
              <Stack gap={3} direction="horizontal">
                <Stack gap={3} direction="horizontal">
                  <div className={`btn btn-lg btn-icon btn-neutral rounded-pill gap-3 ${selectedInvoice === id ? 'bg-dark-subtle' : ''}`}>
