...
const ChatContentHeader = ({ selectedChat, onDeleteChat, setSelectedChat, setContainerToggle, sidebarLeftToggle, setSidebarLeftToggle, sidebarRightToggle, setSidebarRightToggle, }) => {
    ...
    return (<>
      <Stack direction="horizontal" className="ps-2 ps-md-3 pe-2 pe-md-3 border-bottom justify-content-between" style={{ minHeight: '4.5rem' }}>
        <Stack direction="horizontal" gap={2}>
          <Button variant="light" className="btn-icon" onClick={handleGoBack}>
            <i className="fi fi-rr-arrow-left"></i>
          </Button>
          <Stack direction="horizontal" gap={3} onClick={handleSidebarRightToggle} style={{ cursor: 'pointer' }}>
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={toggleSound}>
                {isSoundOn ? (<i className="fi fi-rr-volume"></i>) : (<i className="fi fi-rr-volume-slash"></i>)}
