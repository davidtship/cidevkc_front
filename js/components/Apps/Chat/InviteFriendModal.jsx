...
const InviteFriendModal = ({ show, inviteEmail, setInviteEmail, handleInviteClose, }) => {
    ...
    return (<Modal centered show={show} onHide={handleInviteClose}>
      <Modal.Header closeButton>
        <Modal.Title>Invite Friends</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {validationError && (<Alert variant="danger" className="mb-3 text-danger" style={{ borderStyle: 'dashed', padding: '0.75rem 1rem' }}>
            {validationError}
          </Alert>)}
        <Select isMulti isSearchable placeholder="Select to invite...." options={CustomImageOptions} classNamePrefix="select" components={{
        ...
        <div className="d-md-flex align-items-center gap-4 mb-4">
          ...
          <div className="ms-7 mt-2 ms-md-auto mt-md-0" style={{ minWidth: '150px' }}>
            <Select options={InviteOptions} classNamePrefix="select" defaultValue={defaultEditorOption ? [defaultEditorOption] : []}/>
          </div>
        </div>
        <div className="d-md-flex align-items-center gap-4">
          ...
          <div className="ms-7 mt-2 ms-md-auto mt-md-0" style={{ minWidth: '150px' }}>
            <Select options={InviteOptions} classNamePrefix="select" defaultValue={defaultViewerOption ? [defaultViewerOption] : []}/>
          </div>
        </div>
      </Modal.Body>
      <Modal.Footer>
        ...
        <Button variant="primary" onClick={handleInviteSubmit} className="text-white" disabled={loading}>
          {loading ? (<>
