    ...
    return (<>
      <PerfectScrollbar className="h-100 d-flex flex-column">
        <ListGroup variant="flush" className="flex-grow-1">
          {filteredChatItems.length === 0 ? (<ListGroup.Item className="py-4 text-center">No items found</ListGroup.Item>) : (filteredChatItems.map(({ image, name, message, time, badge, id }, idx) => (<ListGroup.Item className={`py-4 ${selectedChat === chatItems[idx] ? 'bg-secondary-subtle' : ''}`} key={id} onClick={() => handleChatItemClick(idx)} style={{ cursor: 'pointer' }}>
                <Stack direction="horizontal" gap={3} className="align-items-start">
                  ...
                  <Stack className="ms-auto align-items-end">
                    ...
                    {badge && (<span className={`badge bg-${badge[0].color}-subtle text-${badge[0].color}`}>
                        {badge[0].count}
                      </span>)}
                  </Stack>
                </Stack>
