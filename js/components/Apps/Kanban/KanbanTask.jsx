...
const KanbanTask = ({ id, content, index, onDelete }) => {
    ...
    return (<>
      <Draggable key={id} draggableId={id} index={index}>
        {(provided) => (<Card key={id} ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className="mb-3" onClick={openTaskDetailsModal}>
            <Card.Body>
              ...
              <Stack direction="horizontal" gap={3} className="my-4">
                <Stack direction="horizontal" gap={1}>
                  {content[0]?.badge &&
                content[0]?.badge.map((badge, index) => (<Badge key={index} bg="" className={`bg-${badge.toLowerCase()}-subtle text-${badge.toLowerCase()}`}>
                        {badge}
                      </Badge>))}
                </Stack>
              ...
              <Stack direction="horizontal" className="justify-content-between">
                <div className="avatar-group avatar-group-sm">
                  {content[0]?.avatars &&
                content[0]?.avatars.map((avatar, index) => (<div key={index} className="avatar avatar-sm">
      ...
      <TaskDetailsModal isOpen={isTaskDetailsModalOpen} onClose={closeTaskDetailsModal} taskDetails={content[0]} onDelete={onDelete}/>
    </>);
};
