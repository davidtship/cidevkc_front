    ...
    return (<>
      <div style={{ minWidth: '300px', maxWidth: '300px' }}>
        <div className="border border-top-0 rounded mb-3">
          <Stack direction="horizontal" className={`ps-3 pe-2 py-2 gap-3 gap-md-4 bg-body-tertiary rounded border-top border-3 border-${selectedBorderColor}`}>
            <h6 className="mb-0">
        ...
        <Droppable droppableId={id}>
          {(provided) => (<div ref={provided.innerRef} {...provided.droppableProps} className="TaskList">
              {tasks.map((task, index) => (<KanbanTask key={task.id} id={task.id} content={task.content} index={index} onDelete={() => handleDeleteTask(task.id)}/>))}
              {provided.placeholder}
            </div>)}
        </Droppable>
        <Button variant="" className="btn-md bg-body-tertiary border w-100 mb-4" onClick={handleAddTask}>
          <i className="fi fi-rr-plus fs-12"></i>
          <span className="ms-3">Add Task</span>
        </Button>
      </div>
      {isEditModalOpen && (<KanbanColumnEditModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} columnName={newColumnName} onColumnNameChange={handleTitleChange} onSave={handleEditSave} handleClose={() => setIsEditModalOpen(false)} handleSave={() => { }} handleTitleChange={handleTitleChange} handleColorChange={handleColorChange} editingColumn={{
                id,
                title: newColumnName,
