...
const TaskDetailsModal = ({ isOpen, onClose, onDelete, taskDetails, }) => {
    ...
    return (<Modal show={isOpen} onHide={onClose} centered>
      <Modal.Header closeButton>
        <Modal.Title className="hstack gap-3">
          {tasktitle}
          {badge && (<Stack direction="horizontal" gap={1}>
              {badge.map((badge, index) => (<Badge key={index} bg="" className={`bg-${badge.toLowerCase()}-subtle text-${badge.toLowerCase()}`}>
                  {badge.toUpperCase()}
                </Badge>))}
            </Stack>)}
        </Modal.Title>
      ...
      <Modal.Body>
        ...
        <Stack direction="horizontal" className="my-4">
          ...
          {avatars && (<div className="avatar-group avatar-group-sm">
              {avatars.map((avatar, index) => (<div key={index} className="avatar avatar-sm">
      ...
      <Modal.Footer>
        ...
        <Button variant="danger" className="text-white" onClick={handleDelete} disabled={isDeleting}>
          {isDeleting ? (<>
