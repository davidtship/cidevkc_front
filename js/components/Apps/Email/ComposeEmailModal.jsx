import { CustomMultiValue, CustomSelectOption, } from '@/components/UiElements/Extended/Selects/Components/CustomMultipleComponent';
    ...
    return (<>
      <Modal show={show} onHide={onHide} centered size="lg">
        ...
        <Modal.Body>
          <Form>
            <Form.Group className="mb-4 position-relative w-100">
              ...
              <Select isMulti isSearchable placeholder="" options={CustomImageOptions} classNamePrefix="select" components={{
            Option: CustomSelectOption,
            MultiValue: CustomMultiValue,
        }} value={selectedToOption} onChange={(selectedOption) => setSelectedToOption(selectedOption)} className={toError ? 'is-invalid' : ''}/>
              {toError && <Form.Control.Feedback type="invalid">{toError}</Form.Control.Feedback>}
            </Form.Group>
            {isCCVisible && (<Form.Group className="mb-4">
                <Form.Label>Cc</Form.Label>
                <Select isMulti isSearchable placeholder="" options={CustomImageOptions} classNamePrefix="select" components={{
                Option: CustomSelectOption,
                MultiValue: CustomMultiValue,
            }} value={selectedCCOption} onChange={(selectedOption) => setSelectedCCOption(selectedOption)} className={ccError ? 'is-invalid' : ''}/>
                {ccError && <Form.Control.Feedback type="invalid">{ccError}</Form.Control.Feedback>}
              </Form.Group>)}

            {isBCCVisible && (<Form.Group className="mb-4">
                <Form.Label>Bcc</Form.Label>
                <Select isMulti isSearchable placeholder="" options={CustomImageOptions} classNamePrefix="select" components={{
                Option: CustomSelectOption,
                MultiValue: CustomMultiValue,
            }} value={selectedBCCOption} onChange={(selectedOption) => setSelectedBCCOption(selectedOption)} className={bccError ? 'is-invalid' : ''}/>
                {bccError && (<Form.Control.Feedback type="invalid">{bccError}</Form.Control.Feedback>)}
              </Form.Group>)}
            <Form.Group className="mb-4">
              <Form.Label>Subject</Form.Label>
              <Form.Control type="text" value={subject} onChange={(e) => setSubject(e.target.value)} isInvalid={!!subjectError}/>
              <Form.Control.Feedback type="invalid">{subjectError}</Form.Control.Feedback>
            </Form.Group>
            <Form.Group>
              <Form.Label>Message</Form.Label>
              <ReactQuill value={body} onChange={(value) => setBody(value)} modules={quillModules} formats={quillFormats} theme="snow" className={bodyError ? 'is-invalid' : ''}/>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleCompose} className="text-white" disabled={loading}>
            {loading ? (<>
