...
const EmailList = ({ filteredEmails, selectedEmail, handleEmailItemClick, setLoading, setFilteredEmails, setContainerToggle, setSidebarMiniToggle, }) => {
    ...
    return (<>
      <PerfectScrollbar className="h-100 d-flex flex-column">
        {filteredEmails.length === 0 ? (<ListGroup.Item className="py-4 text-center flex-grow-1">No items found</ListGroup.Item>) : (<ListGroup variant="flush" className="flex-grow-1">
            {filteredEmails.map((email, idx) => (<ListGroup.Item className={`py-4 ${selectedEmail && selectedEmail.id === email.id ? 'bg-secondary-subtle' : ''}`} key={idx} onClick={() => handleEmailtemClick(email)} style={{ cursor: 'pointer' }}>
                  ...
                  <div className="w-100">
                    <Stack direction="horizontal" className="w-100 align-items-start mb-1">
                      ...
                      <Stack direction="horizontal" gap={2} className="ms-auto text-end">
                        {email.attachment && (<Link to="">
                            <i className="fi fi-rr-paperclip-vertical fs-14 text-muted"></i>
                          </Link>)}
                        {email.starred && (<Link to="" className="text-warning">
                    ...
                    <div>
                      <p className="fw-light mw-100" style={{
                    overflow: 'hidden',
                    display: '-webkit-box',
