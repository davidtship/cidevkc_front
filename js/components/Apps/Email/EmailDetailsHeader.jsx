...
const EmailDetailsHeader = ({ onDelete, handlePrev, handleNext, selectedEmail, selectedEmailIndex, filteredEmails, onStarChange, setFilteredEmails, setSelectedEmail, setContainerToggle, sidebarLeftToggle, setSidebarLeftToggle, }) => {
    ...
    return (<>
      <Stack gap={4} direction="horizontal" className="w-100 px-3 border-bottom align-items-center justify-content-between" style={{ minHeight: '4.5rem' }}>
        <Stack gap={2} direction="horizontal">
          <Button variant="light" className="btn-icon btn-md" onClick={handleGoBack}>
            <i className="fi fi-rr-arrow-left"></i>
          </Button>
          <div className="fs-14 fw-semibold text-dark text-truncate d-none d-sm-flex" style={{ maxWidth: '180px' }}>
            {subject}
          </div>
          {label && typeof label === 'object' && (<div>
              <span className="d-inline-flex align-items-center gap-2 rounded-pill border py-1 px-2">
                <span className={`d-block rounded-circle bg-gradient bg-${label.gradient}`} style={{ width: '7px', height: '7px' }}></span>
                <span className="fs-12">{label.name}</span>
              </span>
            </div>)}
        </Stack>
        <Stack direction="horizontal" gap={1} className="justify-content-end">
          <Button variant="light" className="btn-icon btn-md d-none d-lg-flex" onClick={handleSidebarLeftToggle}>
          ...
          <OverlayTrigger overlay={<Tooltip>Prev</Tooltip>}>
            <Button variant="light" className="btn-icon btn-md" onClick={handlePrev} disabled={selectedEmailIndex <= 0}>
              <i className="fi fi-rr-angle-left"></i>
            </Button>
          </OverlayTrigger>
          <OverlayTrigger overlay={<Tooltip>Next</Tooltip>}>
            <Button variant="light" className="btn-icon btn-md" onClick={handleNext} disabled={selectedEmailIndex >= filteredEmails.length - 1}>
              <i className="fi fi-rr-angle-right"></i>
            </Button>
          </OverlayTrigger>
          <Dropdown id="dropdown-email-actions">
            <Dropdown.Toggle variant="light" id="dropdown-email-options" className="btn-icon btn-md arrow-none">
