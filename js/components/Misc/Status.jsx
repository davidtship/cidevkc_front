...
const Status = ({ status, changeStatus }) => {
    ...
    const handleStatusChange = (newStatus) => {
        ...
        toast((t) => (<span>
          <span className={`d-inline-block me-2 bg-${statusColor} rounded-circle`} style={{ width: '12px', height: '12px' }}></span>
    ...
    return (<>
      <Dropdown>
        <Dropdown.Toggle split variant="" size="sm" className="px-0">
          <span className={`me-1 bg-${statusOptions.find((opt) => opt.label === status)
            ?.color} rounded-circle`} style={{ width: '6px', height: '6px' }}></span>
          <span className="fs-12 text-muted me-1">{status}</span>
        </Dropdown.Toggle>
        <Dropdown.Menu style={{ width: '100%', maxWidth: '12rem' }}>
          {statusOptions.map((option) => (<Dropdown.Item key={option.label} onClick={() => handleStatusChange(option.label)}>
              <span className={`d-inline-block bg-${option.color} rounded-circle me-2`} style={{ width: '6px', height: '6px' }}></span>{' '}
              {option.label}
            </Dropdown.Item>))}
        </Dropdown.Menu>
      </Dropdown>
