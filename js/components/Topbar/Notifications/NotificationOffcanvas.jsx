...
const NotificationOffcanvas = ({ show, handleClose, toggleSound, isSoundOn, }) => {
    return (<Offcanvas placement="end" show={show} onHide={handleClose} style={{ width: '100%', maxWidth: '460px' }}>
      <Offcanvas.Header className="d-flex align-items-center px-4 border-bottom" style={{ height: '4.5rem' }} closeButton>
        <Offcanvas.Title>Notifications</Offcanvas.Title>
        <div className="ms-auto hstack gap-1">
          <Button variant="light" onClick={toggleSound} className="btn-icon">
            {isSoundOn ? (<i className="fi fi-rr-volume"></i>) : (<i className="fi fi-rr-volume-slash text-danger"></i>)}
          </Button>
          <Dropdown id="dropdown-contact-actions">
            <Dropdown.Toggle variant="light" id="dropdown-contact-options" className="btn-icon arrow-none">
      ...
      <Offcanvas.Body className="p-0 overflow-hidden">
        <Tab.Container defaultActiveKey="alls">
          <Nav variant="tabs" className="nav-line-tabs my-3">
            {navItems.map(({ key, eventKey, label, badgeColor, badgeTextColor, badgeValue }) => (<Nav.Item key={key}>
          ...
          <PerfectScrollbar className="d-flex flex-column position-relative" style={{ maxHeight: 'calc(100vh - 9rem)' }}>
            <Tab.Content className="flex-grow-1">
              {tabPanes.map(({ eventKey, notifications }) => (<Tab.Pane key={eventKey} eventKey={eventKey}>
                  <ListGroup variant="flush">
                    {notifications.map(({ avatarSrc, sender, message, time, category, actions }, index) => (<ListGroup.Item key={index} className="p-4">
                          <Stack direction="horizontal" gap={3} className="w-100 align-items-start">
                            ...
                            <div className="w-100">
                              <Stack direction="horizontal" className="w-100 align-items-start mb-2">
                                <div className="me-4">
                              ...
                              <Stack direction="horizontal" gap={2} className="text-muted mb-3">
                                ...
                                <span className="d-inline-block bg-dark bg-opacity-25 rounded-circle" style={{ width: '0.25rem', height: '0.25rem' }}></span>
                                <small className="text-lowercase">{category}</small>
                              </Stack>
                              {actions && (<Stack direction="horizontal" gap={2} className="text-muted">
                                  {actions.map(({ variant, label, onClick }, actionIndex) => (<div key={actionIndex} className={`btn btn-sm py-1 btn-${variant}`} onClick={onClick}>
                                      {label}
                                    </div>))}
                                </Stack>)}
                            </div>
