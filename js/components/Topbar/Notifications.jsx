...
const Notifications = () => {
    ...
    return (<>
      <Dropdown show={dropDownOpen} onToggle={toggleDropDown}>
        <Dropdown.Toggle as="a" className="arrow-none header-btn" data-bs-toggle="dropdown" data-bs-auto-close="outside" role="button" onClick={toggleDropDown}>
          <i className="fi fi-rr-bell"/>
          <Badge bg="primary" className="rounded-circle position-absolute translate-middle-y top-30 start-40">
            5
          </Badge>
        </Dropdown.Toggle>
        <Dropdown.Menu align="end" className="dropdown-xl py-0 px-0 overflow-hidden">
          <div className="p-0 border-top-0 border-start-0 border-end-0 border-dashed border" onClick={toggleDropDown}>
          ...
          <PerfectScrollbar style={{ maxHeight: 320 }} className="list-group list-group-flush position-relative">
            {notify.map(({ variant, icon, title, createdAt }, idx) => {
            return (<Link key={idx} to="" onClick={openOffcanvas} className="list-group-item fw-normal d-flex position-relative">
          ...
          <div className="px-4 py-3 border-top d-flex justify-content-between align-items-center">
            <Link to="#" onClick={toggleSound} className="hstack gap-2">
              {isSoundOn ? (<span className="hstack gap-2">
                  <i className="fi fi-rr-volume"></i>
                  <span>Mute</span>
                </span>) : (<span className="hstack gap-2 text-danger">
      ...
      {showOffcanvas && (<NotificationOffcanvas show={showOffcanvas} handleClose={handleCloseOffcanvas} toggleSound={toggleSound} isSoundOn={isSoundOn}/>)}
    </>);
};
