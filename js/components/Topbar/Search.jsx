...
const SearchLink = ({ to, label }) => (<Link to={to} className="fw-medium px-2 border rounded hstack gap-4 mb-2" style={{ paddingTop: '0.45rem', paddingBottom: '0.45rem' }}>
      ...
      <Modal show={searchOpen} onHide={toggleSearch} className="search-modal">
        <Modal.Header>
          <Form className="w-100">
            <InputGroup>
              ...
              <Form.Control className="border-0 fw-medium shadow-none" placeholder="Search...." value={searchTerm} onChange={(e) => handleSearch(e.target.value)}/>
        ...
        <div className="p-4 overflow-x-hidden" style={{ maxHeight: '400px' }}>
          {searchResults.length === 0 ? (<>
              {searchTerm ? (<h5 className="text-muted my-4">
            ...
            </>) : (searchResults.map((parentItem) => (<React.Fragment key={parentItem.key}>
                {parentItem.isTitle && (<h6 className="fs-11 text-muted text-uppercase mb-3 mt-4">{parentItem.label}</h6>)}
                {parentItem.isTitle ? null : (<SearchLink to={parentItem.url || ''} label={parentItem.label}/>)}
                {parentItem.children &&
                parentItem.children.map((childItem) => (<SearchLink key={childItem.key} to={childItem.url || ''} label={childItem.label}/>))}
              </React.Fragment>)))}
        </div>
        <Modal.Footer>
