...
const Profile = () => {
    ...
    return (<>
      <Dropdown show={dropDownOpen} onToggle={toggleDropDown}>
        <Dropdown.Toggle className="arrow-none header-btn px-2" as="a" role="button" onClick={toggleDropDown}>
          <Avatar type="image" src={path} size="md">
            <div className={`avatar-status bg-${statusOptions.find((opt) => opt.label === status)?.color}`}></div>
          </Avatar>
        </Dropdown.Toggle>
        <Dropdown.Menu align="end" className="dropdown-md px-0 py-0">
          ...
          <div className="px-2 pt-2">
            {profileDataGroup1.map(({ redirectTo, icon, label }, idx) => (<Link to={redirectTo} className="dropdown-item" key={idx}>
                <i className={icon}></i>
          ...
          <div className="px-2 pb-2">
            {profileDataGroup2.map(({ redirectTo, icon, label }, idx) => (<Link to={redirectTo} className="dropdown-item text-danger" key={idx}>
                <i className={icon}></i>
                <span className="ms-3">{label}</span>
              </Link>))}
          </div>
