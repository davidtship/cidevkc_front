...
const MegaMenuApps = ({ isOpen, setIsOpen }) => {
    const megaMenuItemsClose = () => {
        setIsOpen(false);
    };
    return (<>
      <Dropdown show={isOpen} onMouseEnter={() => setIsOpen(true)} onMouseLeave={() => setIsOpen(false)} className="megumenu-dropdown megamenu-xl">
        <Dropdown.Toggle variant="" className={`btn-megumenu arrow-none ${isOpen ? 'show' : ''}`} id="mega-menu-apps" aria-expanded={isOpen ? 'true' : 'false'}>
          Apps
        </Dropdown.Toggle>
        <Dropdown.Menu className="megumenu-wrapper">
          ...
          <Row className="g-12">
            <Col lg={8}>
              <Row className="g-4">
                {megaAppsItems.map(({ to, title, desc, icon }, idx) => (<Col lg={6} key={idx}>
                    <Link to={to} className="megumenu-item">
                      <div className="d-flex gap-4">
                        <div className="d-flex flex-shrink-0 align-items-center justify-content-center rounded bg-secondary-subtle text-dark" style={{ width: '3rem', height: '3rem' }}>
            ...
            <Col lg={4}>
              <div className="p-4 d-flex align-items-center justify-content-center bg-secondary-subtle rounded-3" style={BgCircleShapeStyle}>
