...
const Helpdesk = () => {
    ...
    return (<>
      <Dropdown show={dropDownOpen} onToggle={toggleDropDown}>
        <Dropdown.Toggle as="a" className="arrow-none header-btn" role="button" onClick={toggleDropDown}>
          <i className="fi fi-rr-interrogation"/>
        </Dropdown.Toggle>
        <Dropdown.Menu align="end" className="dropdown-xl py-0 px-0 overflow-hidden">
          <div className="p-2 border-top-0 border-start-0 border-end-0 border-dashed border bg-primary text-white text-center px-6 py-12" onClick={toggleDropDown} style={BgCircleShapeStyle}>
            ...
            <Button className="fs-10 fw-semibold py-2 px-3 bg-body text-dark text-uppercase rounded-pill" style={{ letterSpacing: '1px' }}>
              Get Support
            </Button>
          </div>
          <PerfectScrollbar style={{ maxHeight: '320px' }}>
            <Row className="g-0 px-4 pt-4 pb-2">
              <Col>
                ...
                {applications.map(({ to, label }) => (<Link key={to} to={to} className="px-0 dropdown-item">
                    {label}
                  </Link>))}
              </Col>
              <Col>
                <p className="fs-11 fw-semibold text-uppercase text-muted mb-1">Helpdesk</p>
                {docs.map(({ to, label }) => (<Link key={to} to={to} className="px-0 dropdown-item">
                    {label}
                  </Link>))}
                <p className="fs-11 fw-semibold text-uppercase text-muted mt-4 mb-1">Shoutcuts</p>
                {shoutcuts.map(({ to, label }) => (<Link key={to} to={to} className="px-0 dropdown-item">
                    {label}
                  </Link>))}
              </Col>
            </Row>
