...
const IntegrationsConnect = () => {
    ...
    return (<>
      ...
      <Row className="g-3 g-md-4">
        {connectedData.map((item, idx) => (<Col md={6} xl={4} key={idx}>
            <Card>
              <Card.Body>
                <Stack direction="horizontal" className="mb-4">
                  <Stack gap={3} direction="horizontal">
                    <div className="p-2 border rounded bg-secondary-subtle" style={{ width: '3rem', height: '3rem' }}>
                  ...
                  <div className="ms-auto">
                    <FormCheck type="switch" id={`syncSwitch-${idx}`} checked={syncStates[item.name]} onChange={() => handleToggleSwitch(item.name)}/>
                  </div>
                </Stack>
      ...
      <Modal centered show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Body>
          {selectedIntegration && (<>
              <Stack direction="horizontal" className="mb-4">
                ...
                <div className="ms-auto">
                  <span className={`fs-12 px-2 ms-2 border rounded-pill hstack ${selectedIntegration.status === 'Verified'
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" className="text-white" onClick={handleUninstallModal} disabled={isSaving}>
            {isSaving ? (<>
