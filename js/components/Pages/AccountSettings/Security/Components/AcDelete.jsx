...
const AcDelete = () => {
    ...
    return (<>
      ...
      <Alert variant="danger" className="text-danger border border-danger border-opacity-10 mb-4 fs-12 fst-italic">
      ...
      <Form>
        ...
        <Form.Group controlId="formCheckbox" className="mt-3">
          <Form.Check type="checkbox" label="I confirm my account deletions or deactivation." checked={isChecked} onChange={handleCheckboxChange}/>
        </Form.Group>
        <div className="d-grid d-sm-flex gap-1 mt-4">
          <Button variant="danger" type="button" onClick={handleDeleteAccount} disabled={!password || !isChecked}>
            Delete Account
          </Button>
          <Button variant="warning" type="button" onClick={handleDeactivateAccount} disabled={!password || !isChecked}>
            Deactivate Account
          </Button>
        </div>
      </Form>
