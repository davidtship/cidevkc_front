...
const BillingMethod = () => {
    const renderPaymentMethods = () => {
        return paymentMethods.map(({ name, image, number, expireDate, lastUsed, ifDefault }, index) => (<ListGroup.Item key={index} className="d-sm-flex align-items-center justify-content-between gap-4">
        <div className="d-sm-flex align-items-center gap-4">
          ...
          <div className="radio-card-content">
            ...
            {expireDate && (<span className="text-muted">
                Expire date: <span className="fw-medium">{expireDate}</span>
              </span>)}
            {lastUsed && (<span className="text-muted">
        ...
        <Stack direction="horizontal" className="gap-4 mt-3 mt-sm-0">
          ...
          {!ifDefault && (<Link to="#!" className="text-primary">
              Set as default
            </Link>)}
          <Link to="#!">
