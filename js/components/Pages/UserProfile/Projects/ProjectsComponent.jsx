...
const ProjectsComponent = () => {
    return (<>
      <Row className="g-3 g-md-4">
        {projectsData.map(({ color, badge, name, client, startDate, endDate, desc, progress, avatars, attach, comment, }, idx) => (<Col sm={6} xxl={4} key={idx}>
              <Card>
                <Card.Body>
                  ...
                  <div className="my-6">
                    ...
                    <ProgressBar now={progress} variant={`w-100 rounded bg-${color}`} style={{ height: '5px' }}/>
                  </div>
                  <Stack direction="horizontal" className="mt-3 justify-content-between">
                    <div className="avatar-group avatar-group-sm">
                      {avatars.map((avatar, idx) => (<Avatar key={idx} type="image" src={avatar} alt={`User Avatar ${idx + 1}`} className="avatar avatar-sm"/>))}
                    </div>
