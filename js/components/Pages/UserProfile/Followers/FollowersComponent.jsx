...
const FollowersComponent = () => {
    ...
    return (<>
      <Row className="g-3 g-md-4">
        {followerData.map(({ image, name, email, isFollowing }, idx) => (<Col sm={6} lg={4} xxl={3} key={idx}>
            <Card>
              <Card.Body className="text-center py-8">
                <div className="mb-4">
                  <Avatar type="image" src={image} alt="User Avatar" className="avatar avatar-xxl"/>
                </div>
                <div>
                  <h6 className="fw-semibold mb-1">{name}</h6>
                  <p className="fs-13 fw-light text-muted mb-3">{email}</p>
                  <Button variant="soft-light" className={`rounded-pill ${isFollowing ? 'bg-success-subtle text-success' : ''}`} onClick={() => toggleIsFollow(idx)}>
