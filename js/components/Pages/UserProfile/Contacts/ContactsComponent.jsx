...
const ContactsComponent = () => {
    ...
    return (<>
      <Row className="g-3 g-md-4">
        {contactsData.map(({ image, name, email, phone, isFavorite }, idx) => (<Col sm={6} lg={4} xxl={3} key={idx}>
            <Card>
              <Card.Body>
                <Stack gap={3} direction="horizontal" className="justify-content-between">
                  <Stack gap={3} direction="horizontal">
                    <Avatar type="image" src={image} alt="User Avatar" className="avatar avatar-xl"/>
                    <div>
                  ...
                  <Link to="#!" className={`${isFavorite ? 'text-warning' : ''}`} onClick={() => toggleIsFollow(idx)}>
                    {isFavorite ? (<i className="fi fi-rr-star text-warning"></i>) : (<i className="fi fi-rr-star"></i>)}
                  </Link>
                </Stack>
              </Card.Body>
            </Card>
