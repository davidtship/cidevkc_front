...
const CommonFooter = () => {
    return (<>
      <Card.Footer className="px-4 px-6 py-12 py-lg-16">
        <div className="mb-6 mx-auto mx-md-0 text-center text-md-start" style={{ maxWidth: '425px' }}>
          <div className="mx-auto mx-md-0 border border-2 border-primary rounded-pill mb-4" style={{ width: '3rem' }}></div>
        ...
        <Row className="g-4 align-items-center">
          <Col md={8} lg={6}>
            <Stack gap={4} direction="horizontal" className="flex-wrap justify-content-center justify-content-md-start">
              {links.map(({ to, text }, idx) => (<Link key={idx} to={to}>
                  {text}
                </Link>))}
            </Stack>
          </Col>
          <Col md={4} lg={6}>
            <Stack gap={1} direction="horizontal" className="justify-content-center justify-content-md-end">
              {socialMediaIcons.map(({ iconClass }, idx) => (<Button key={idx} variant="" className="btn-icon">
