        ...
        <Row>
          <Col lg={6}>
            <Accordion className="accordion-icon" defaultActiveKey="0">
              {faqs1.map(({ question, answer }, idx) => (<Accordion.Item key={idx} eventKey={(idx + 1).toString()} className="border-top mt-2 rounded-top">
          ...
          <Col lg={6}>
            <Accordion className="accordion-icon" defaultActiveKey="0">
              {faqs2.map(({ question, answer }, idx) => (<Accordion.Item key={idx} eventKey={(idx + 1).toString()} className="border-top mt-2 rounded-top">
                  <Accordion.Header>{question}</Accordion.Header>
                  <Accordion.Body>{answer}</Accordion.Body>
                </Accordion.Item>))}
            </Accordion>
