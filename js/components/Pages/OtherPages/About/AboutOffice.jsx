...
const AboutOffice = () => {
    ...
    const renderAddress = (address) => <div dangerouslySetInnerHTML={{ __html: address }}/>;
    return (<>
      <div className="pt-8 pt-md-12 pt-lg-16 px-4 px-md-6 bg-secondary-subtle rounded-3">
        <Row>
          ...
          <Col sm={6} lg={3}>
            {column1.map(({ city, address }, idx) => (<div key={idx} className="mb-6">
                <h5>{city}</h5>
                {renderAddress(address)}
              </div>))}
          </Col>
          <Col sm={6} lg={3}>
            {column2.map(({ city, address }, idx) => (<div key={idx} className="mb-6">
                <h5>{city}</h5>
                {renderAddress(address)}
              </div>))}
          </Col>
