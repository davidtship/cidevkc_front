    ...
    return (<>
      <div className="mb-8 mb-md-12 mb-lg-16 text-center mx-auto" style={{ maxWidth: '600px' }}>
        <div className="d-inline-block border rounded-pill">
          <Link to="#!" className={`btn rounded-pill ${billingOption === 'monthly' ? 'active' : ''}`} onClick={() => handleBillingOptionChange('monthly')}>
            Monthly billing
          </Link>
          <Link to="#!" className={`btn rounded-pill ${billingOption === 'annual' ? 'active' : ''}`} onClick={() => handleBillingOptionChange('annual')}>
            Annual billing
          </Link>
        </div>
      </div>
      <Table responsive bordered>
        <thead>
          <tr>
            <th></th>
            {plans[billingOption].map(({ type, price, features }, idx) => (<th key={idx} style={commonWidth} className="text-wrap">
        ...
        <tbody>
          {featuresData.map(({ name, basic, business, enterprise }, idx) => (<React.Fragment key={idx}>
              <tr>
                <td className="hstack">
