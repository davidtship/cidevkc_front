          ...
          {campaigns.map((campaign) => (<tr key={campaign.id}>
              ...
              <td>
                <div className="d-flex gap-1">
                  {campaign.progress.map((p, index) => (<div key={index} className={`bg-${p ? campaign.state : 'secondary'} rounded-pill opacity-${p ? 75 : 25}`} style={{ width: '1.5rem', height: '0.25rem' }}></div>))}
                </div>
              </td>
              <td className="text-end">
