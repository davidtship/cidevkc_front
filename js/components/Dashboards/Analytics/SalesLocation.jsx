...
const SalesLocation = ({ width, height }) => {
    ...
    return (<>
      <Card>
        ...
        <ListGroup>
          {countries.map(({ name, color, count }, index) => (<ListGroup.Item key={index} className="hstack py-2">
              <div className="me-auto">
                <Link to="" className="hstack gap-3">
                  <span className={`bg-${color} rounded-circle d-flex flex-shrink-0`} style={{ width: '0.45rem', height: '0.45rem' }}></span>
                  <span>{name}</span>
                </Link>
              </div>
