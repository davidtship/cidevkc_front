...
const MeetingItem = ({ date, month, title, time, joinButtonVariant, joinButtonText, badgeDate, badgeColor, badgeText, }) => {
    ...
    return (<ListGroup.Item className="py-5">
      <div className="d-flex gap-4">
        <div className={`p-2 rounded-2 bg-${badgeColor}-subtle text-${badgeColor} text-center flex-shrink-0`} style={{ width: '4rem', height: '4rem' }}>
        ...
        <div className="w-100">
          <Stack direction="horizontal" gap={2}>
            ...
            <Button variant={joinButtonVariant} className="btn-xs ms-auto text-white" onClick={handleJoinClick} disabled={loading || joined}>
              {loading ? (<span>Joining...</span>) : joined ? (<span>Joined</span>) : (<>
          ...
          <div className="d-flex gap-2">
            ...
            <span className="p-2 hstack gap-2 badge text-body-secondary border rounded-pill">
              <span className={`rounded-circle d-inline-block bg-${badgeColor}`} style={{ width: '0.5rem', height: '0.5rem' }}></span>
              <span>{badgeText}</span>
            </span>
          </div>
        </div>
      ...
      <Card>
        ...
        <Card.Body className="p-0">
          <ListGroup variant="flush">
            {meetings.map((meeting, index) => (<MeetingItem key={index} {...meeting}/>))}
          </ListGroup>
        </Card.Body>
