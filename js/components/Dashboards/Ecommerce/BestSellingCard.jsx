    ...
    return (<>
      <Card>
        ...
        <ListGroup variant="flush">
          {products.map((product) => (<ListGroup.Item className="hstack" key={product.id} style={{ borderBottomStyle: 'dashed' }}>
              <Stack direction="horizontal" gap={3}>
                <div style={{ width: '3.5rem', height: '3rem' }}>
                  <Avatar size="md" type="image" shape="1" src={product.img} alt={`Product ${product.id} Avatar`} className="w-100 flex-shrink-0 rounded bg-body-secondary"/>
                </div>
                <Link to="">
                  <h6>{product.name}</h6>
              ...
              <div className="ms-auto text-end">
                <Stack direction="horizontal" gap={1} className="mb-2">
                  {[...Array(5)].map((_, index) => (<i key={index} className="fi fi-sr-star fs-12 text-warning"></i>))}
                </Stack>
