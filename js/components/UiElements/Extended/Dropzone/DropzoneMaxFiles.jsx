...
const DropzoneBasic = () => {
    ...
    const files = acceptedFiles.map((file, index) => (<Stack direction="horizontal" gap={3} key={file.path} className="py-2 px-3 mb-2 border rounded">
      {imagePreviews[index] && (<Avatar type="image" size="md" src={imagePreviews[index]} alt="Preview"/>)}
      <div>
        <h6 className="mb-1">{file.path}</h6>
        <span className="fs-13 text-muted">{formatBytesToMB(file.size)}</span>
      </div>
      <div className="ms-auto">
        <Button type="button" variant="soft-light" className="btn-icon" onClick={() => handleRemoveFile(index)}>
