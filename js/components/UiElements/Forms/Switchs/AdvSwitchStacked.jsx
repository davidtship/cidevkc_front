...
const AdvSwitchStacked = () => {
    return (<>
      {stackedSwitches.map(({ size, name, label }, index) => (<React.Fragment key={index}>
          <Form.Check.Label className={`switch${size ? ` switch-${size}` : ''}`}>
            <Form.Check.Input type="radio" className="switch-input" name={name} defaultChecked={index === 0}/>
      ...
      {stackedSquareSwitches.map(({ size, name, label, className }, index) => (<React.Fragment key={index}>
          <Form.Check.Label className={`switch${size ? ` switch-${size}` : ''}${className ? ` ${className}` : ''}`}>
            <Form.Check.Input type="radio" className="switch-input" name={name} defaultChecked={index === 0}/>
            <span className="switch-toggle-slider">
              <span className="switch-on"></span>
              <span className="switch-off"></span>
