...
const AdvSwitchSizing = () => {
    return (<>
      {switches.map(({ size, label }, index) => (<React.Fragment key={index}>
          <Form.Check.Label className={`switch${size ? ` switch-${size}` : ''}`}>
            <Form.Check.Input type="checkbox" className="switch-input" defaultChecked={size === 'sm'}/>
      ...
      <hr className="my-4"/>
      {squareSwitches.map(({ size, label, className }, index) => (<React.Fragment key={index}>
