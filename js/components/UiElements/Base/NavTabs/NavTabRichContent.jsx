...
const NavTabTheme = () => {
    ...
    return (<>
      {colors.map((variant, idx) => (<Tab.Container defaultActiveKey="home" key={idx}>
          <Nav className={`mb-3 gap-3 align-items-center justify-content-center nav-rich-content-tabs nav-rich-content-${variant.toLowerCase()}`}>
            {tabsData.map(({ eventKey, icon, disabled }, index) => (<Nav.Item key={index}>
          ...
          <hr className="my-4"/>
          <Nav className={`mb-3 gap-3 align-items-center justify-content-center nav-rich-content-tabs nav-rich-content-${variant.toLowerCase()}`}>
            {tabsData.map(({ eventKey, icon, title, disabled }, index) => (<Nav.Item key={index}>
