...
const MiscAlert = () => {
    return (<>
      {alerts.map((alert, index) => (<AlertComponent key={index} {...alert}/>))}
    </>);
};
const AlertComponent = ({ variant, dismissible, closeVariant, style }) => {
    return (<Alert dismissible={dismissible} variant={variant} closeVariant={closeVariant} style={style ?? undefined} className="text-bg-primary text-white">
