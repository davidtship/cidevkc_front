...
const ModalTheme = () => {
    ...
    return (<>
      <Stack direction="horizontal" gap={1} className="flex-wrap">
        {colors.map((variant, idx) => (<Button key={idx} variant="" onClick={modals[idx].handleShow} className={`d-inline-block btn-${variant.toLowerCase()}`}>
            {variant}
          </Button>))}
      </Stack>
      {colors.map((variant, idx) => (<Modal key={idx} show={modals[idx].show} onHide={modals[idx].handleClose} className={`modal-theme-${variant.toLowerCase()}`}>
          ...
          <Modal.Footer>
            <Button variant="" className="btn bg-white text-black" onClick={modals[idx].handleClose}>
              Close
            </Button>
