...
const ModalUsecase = () => {
    ...
    const usecaseModals = [
        ...
        },
        {
          ...
          <form>
            <input type="email" className="form-control" placeholder="Email for password reset link" required/>
          </form>
        </>),
    ...
    return (<>
      <Stack direction="horizontal" gap={1} className="flex-wrap">
        {usecaseModals.map((modal, index) => (<React.Fragment key={index}>
            <Button variant={modal.variant} onClick={() => handleShow(`showUsecase${modal.variant.charAt(0).toUpperCase() + modal.variant.slice(1)}`)}>
              Usecase {modal.variant.charAt(0).toUpperCase() + modal.variant.slice(1)}
            </Button>
            <Modal show={modals[`showUsecase${modal.variant.charAt(0).toUpperCase() + modal.variant.slice(1)}`]} onHide={() => handleClose(`showUsecase${modal.variant.charAt(0).toUpperCase() + modal.variant.slice(1)}`)} centered>
              <Modal.Body className="modal-body text-center mx-auto py-4 px-4 py-md-16 px-md-16">
                <div className={`bg-${modal.variant}-subtle text-${modal.variant} rounded-circle d-flex align-items-center justify-content-center mx-auto mb-5`} style={{ width: '5rem', height: '5rem' }}>
                  <i className={`fi ${modal.icon} fs-2`}></i>
                </div>
                <h3 className={`mb-3 text-${modal.variant}`}>{modal.title}</h3>
                {modal.body}
                <div className="mt-4 mb-6">
                  <Link to="#!" className={`btn btn-${modal.variant}`} style={{ width: '200px' }} onClick={modal.onClick}>
                    {modal.buttonText}
                  </Link>
                </div>
