...
const ModalPosition = () => {
    ...
    return (<>
      <Stack direction="horizontal" gap={1} className="flex-wrap">
        {modalData.map((modal) => (<React.Fragment key={modal.key}>
            <Button variant="primary" onClick={() => handleShow(modal.key)}>
              {modal.buttonText}
            </Button>
            <Modal show={modals[modal.key]} onHide={() => handleClose(modal.key)} className={`modal-dialog-${modal.position}`}>
              <Modal.Body className="text-center mx-auto py-12 p-md-12">
                <div className="bg-success-subtle text-success rounded-circle d-flex align-items-center justify-content-center mx-auto mb-4" style={{ width: '5rem', height: '5rem' }}>
