...
const CardNavigation = () => {
    return (<Row className="g-3 g-md-4">
      {tabData.map(({ defaultActiveKey, navVariant, items, panes }, index) => (<Col lg={6} key={index}>
          <Tab.Container defaultActiveKey={defaultActiveKey}>
            <Card>
              <Card.Header>
                <Nav variant={navVariant}>
                  {items.map(({ eventKey, label, disabled }, idx) => (<Nav.Item key={idx}>
              ...
              <Card.Body>
                <Tab.Content>
                  {panes.map(({ eventKey, content }, idx) => (<Tab.Pane eventKey={eventKey} key={idx}>
                      {content}
                    </Tab.Pane>))}
                </Tab.Content>
              </Card.Body>
