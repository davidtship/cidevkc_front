    ...
    return (<>
      ...
      <Card className={`apps-card ${sidebarMiniToggle ? 'apps-sidebar-mini-toggle' : ''}`}>
        <div className="apps-sidebar-mini">
          <EmailSidebar categories={categories} labels={labels} selectedCategory={selectedCategory} handleCategoryChange={handleCategoryChange} setSidebarMiniToggle={setSidebarMiniToggle}/>
        </div>
        <div className="apps-sidebar apps-sidebar-left">
          <EmailHeader selectedCategory={selectedCategory} labels={labels} categories={categories} sidebarMiniToggle={sidebarMiniToggle} setSidebarMiniToggle={setSidebarMiniToggle}/>
          <EmailSearchBar searchTerm={searchTerm} handleSearch={handleSearch}/>
          <EmailList filteredEmails={filteredEmails} selectedEmail={selectedEmail} handleEmailItemClick={handleEmailItemClick} setLoading={setLoading} setFilteredEmails={setFilteredEmails} setContainerToggle={setContainerToggle} setSidebarMiniToggle={setSidebarMiniToggle}/>
        </div>
        <div className="apps-container">
          <div className="apps-container-inner">
            {loading ? (<Stack className="align-items-center justify-content-center flex-grow-1 h-100 ">
                <Preloader />
              </Stack>) : !selectedEmail ? (<Stack className="d-none d-lg-flex align-items-center justify-content-center flex-grow-1 h-100 mx-auto text-center" style={{ maxWidth: '320px' }}>
                ...
                <Button variant="" className="btn-neutral" onClick={() => setShowComposeModal(true)}>
                  <i className="fi fi-br-plus fs-10"></i>
                  <span className="ms-2">New Message</span>
                </Button>
              </Stack>) : (<>
                <EmailDetailsHeader onDelete={handleDeleteEmail} handlePrev={handlePrev} handleNext={handleNext} selectedEmail={selectedEmail} selectedEmailIndex={selectedEmailIndex} setSelectedEmail={setSelectedEmail} filteredEmails={filteredEmails} onStarChange={handleStarChange} setFilteredEmails={setFilteredEmails} setContainerToggle={setContainerToggle} sidebarLeftToggle={sidebarLeftToggle} setSidebarLeftToggle={setSidebarLeftToggle}/>
