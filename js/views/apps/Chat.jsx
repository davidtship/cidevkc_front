...
const Chat = () => {
    ...
    return (<>
      ...
      <Card className={`apps-card  ${selectedChat ? 'apps-container-toggle' : ''}  ${sidebarLeftToggle ? 'apps-sidebar-left-toggle' : ''} ${sidebarRightToggle ? 'apps-sidebar-right-toggle' : ''}`}>
        <div className="apps-sidebar apps-sidebar-left">
          ...
          <ChatAsiderChatList chatItems={chatItems} selectedChat={selectedChat} setSelectedChat={setSelectedChat} setLoading={setLoading} searchQuery={searchQuery} onDeleteChat={handleDeleteChat} setContainerToggle={setContainerToggle}/>
        </div>
        <div className="apps-container">
          <div className="apps-container-inner">
            {loading ? (<Stack className="align-items-center justify-content-center flex-grow-1 h-100 ">
                <Preloader />
              </Stack>) : !selectedChat ? (<Stack className="d-none d-lg-flex align-items-center justify-content-center flex-grow-1 h-100 mx-auto text-center" style={{ maxWidth: '320px' }}>
                ...
                <ChatContentHeader selectedChat={selectedChat} onDeleteChat={handleDeleteChat} setSelectedChat={setSelectedChat} setContainerToggle={setContainerToggle} sidebarLeftToggle={sidebarLeftToggle} setSidebarLeftToggle={setSidebarLeftToggle} sidebarRightToggle={sidebarRightToggle} setSidebarRightToggle={setSidebarRightToggle}/>
                <ChatContentDetails selectedChat={selectedChat}/>
                <ChatActionFooter onReplyMessage={handleReplyMessage}/>
              </>)}
          </div>
          <ChatUserInfoSidebar selectedChat={selectedChat} setSidebarRightToggle={setSidebarRightToggle}/>
        </div>
        <div className="custom-backdrop" onClick={() => setSidebarRightToggle(false)}></div>
      </Card>
      <InviteFriendModal show={showInviteModal} inviteEmail={inviteEmail} setInviteEmail={setInviteEmail} handleInviteClose={() => setShowInviteModal(false)}/>
    </>);
};
export default Chat;
