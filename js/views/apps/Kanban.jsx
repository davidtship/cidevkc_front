        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <PerfectScrollbar className="apps-scrollable-content">
            <Card.Body className="d-flex align-items-start gap-3 gap-md-4">
              {data.map((column) => (<KanbanColumn key={column.id} id={column.id} title={column.title} color={column.color} tasks={column.tasks} onDelete={() => deleteColumn(column.id)} onEmptyTasks={() => emptyColumnTasks(column.id)} addTaskToColumn={(newTask) => addTaskToColumn(column.id, newTask)}/>))}
            </Card.Body>
          </PerfectScrollbar>
        </DragDropContext>
      </Card>
