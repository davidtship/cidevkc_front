    ...
    return (<>
      ...
      <ErrorLayout>
        ...
        <div className="mb-8 text-start">
          <Stack direction="horizontal" gap={2} className="justify-content-center">
            <Form className="w-100">
              <Form.Control placeholder="Enter your email" value={email} onChange={(e) => {
            ...
            <Button onClick={handleSubscribe} disabled={loading} className="text-white">
              {loading ? (<>
                  <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true" style={{ width: '1.25rem', height: '1.25rem' }}></span>
                </>) : ('Subscribe')}
            </Button>
          </Stack>
        ...
        <Stack gap={1} direction="horizontal" className="justify-content-center">
          {socialMediaIcons.map((icon, index) => (<Button key={index} variant="" className="btn-icon btn-md">
